...
//    resmMobile.show(1000)
    ...
    val outputTmpDir = "/share/cTable_emp_inter_ofo_mobile_temp"
    val output = "odsdb_prd.emp_inter_ofo_mobile_temp"
  ...
  def getProducts(empInterOfoData: DataFrame, sqlContext: HiveContext) = {
    ...
    val mobileEmpInterOfoData = empInterOfoData.select("insured_cert_no","insured_mobile","product_type")
      ...
      .reduceByKey((x1,x2) => {
        val insured_cert_nos = x1._1+"\u0001"+x2._1
        val product_types = x1._2+"\u0001"+x2._2
        (insured_cert_nos,product_types)
      })
      .map(x => {
        val certLength =  x._2._1.trim.split("\u0001").distinct.mkString("\u0001").split("\u0001").length
        if(certLength <=1){
          val product_types = x._2._2.split("\u0001").distinct.mkString("\u0001")
          val certs = x._2._1.trim.split("\u0001").distinct
          if(certLength == 0){
