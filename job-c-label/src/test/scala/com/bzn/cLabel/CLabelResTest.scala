...
//    open_other_policy_temp.insertInto("odsdb_prd.open_other_policy_temp",overwrite = true)
    ...
    val open_other_policy_temp = sqlContext.read.jdbc(location_mysql_url_dwdb,"dim_product",prop)
      ...
      .cache()
    val firstRes = open_other_policy_temp.where("product_new_1 <> 'TMT'").select("product_code","product_new_1")
    val secondRes = open_other_policy_temp.where("product_new_1 = 'TMT'").select("product_code","product_new_2")

    val res = firstRes.unionAll(secondRes).distinct()
//    res.show(1000)
    res.insertInto("odsdb_prd.dim_product_temp",overwrite = true)
