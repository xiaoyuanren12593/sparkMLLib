...
//
    ...
    println(ofoData.count())
//
//    val empInterOfoData2Hive = empData.unionAll(interData).unionAll(ofoData)
//
//    empInterOfoData2Hive.insertInto("odsdb_prd.emp_inter_ofo",overwrite = true)
...
//      .distinct()
  ...
  def get58CourierData(sqlContext : SQLContext) ={
    import sqlContext.implicits._
    val courier58 = sqlContext.sql("select 'courier_name','courier_card_no', client_mobile ,'58速运-司机' from odsdb_prd.open_express_policy_all_d")
      .toDF("insured_name","insured_cert_no","insured_mobile","product_type")
      .map(x => {
        val insured_name = x.getAs[String]("insured_name")
        var insured_cert_no = x.getAs[String]("insured_cert_no")
