...
object MobileInfoDistinctTest {
  ...
  def availMobile (mobilelocation:DataFrame,sqlContext: SQLContext) = {
    ...
    val resMobile = mobilelocation.where("province is not  null")
      .select("id","mobile","data_source","type","province","city","operator","location","create_time")
    ...
    val res = resMobile.map(x => {
      ...
      val create_time = x.getAs[java.sql.Timestamp]("create_time").toString
      (mobile,(id,data_source,`type`,province,city,operator,location,create_time))
    })
      .reduceByKey((x1,x2) =>{
        x1
      })
      ...
      .toDF("id","mobile","data_source","type","province","city","operator","location","create_time")
