  ...
  def main(args: Array[String]): Unit = {
    ...
    res.show(100)
      ...
      .map(x => {
        ...
        val count = x.getAs[Double]("count")
        val charges = x.getAs[Double]("charges")
...
//
//    monthData.rdd.map(x => x.mkString("\001")).repartition(1).saveAsTextFile(outputTmpDir)
...
//       resTwo.show(10)
      ...
      .map(x => {
        ...
        val chargesres = charges.toDouble
  ...
  def getYear(sqlContext:SQLContext)={
    ...
    val resTwo = resOne.join(ods_policy_product_plan,resOne("policy_code") ===ods_policy_product_plan("policy_code_a"))
      ...
      .map(x=> {
        ...
        val charges = numberFormat.format((days/365)*res_sku_price)
