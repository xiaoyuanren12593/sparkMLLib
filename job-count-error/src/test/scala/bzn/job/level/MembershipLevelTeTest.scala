...
object MembershipLevelTest {
  ...
  /**
    ...
    **/
  def read_people_product(sqlContext: HiveContext, location_mysql_url: String, prop: Properties, location_mysql_url_dwdb: String): RDD[(String, (Int, Int))] = {
  ...
  //Hbase企业标签数据
  def getHbase_data(usersRDD: RDD[(ImmutableBytesWritable, Result)], ods_ent_guzhu_salesman: Array[String]): RDD[(String, (String, Double, Double, Int, String))] = {
    val end: RDD[(String, (String, Double, Double, Int, String))] = usersRDD.map { x => {
  ...
  def main(args: Array[String]): Unit = {
    ...
    }).distinct().filter(x => if (ods_ent_guzhu_salesman.contains(x._1)) true else false).persist(StorageLevel.MEMORY_ONLY)
    ...
    //标签数据与每个月的在保人数取值
    val tep_one_res = hbase_result_data.join(ent_people).mapPartitions(par => {
      //将日期小于当前月的找出来
      val now: Date = new Date()
...
//    //    tep_end.foreach(println)
//    val table_name = "mid_guzhu_member_hierarchy_temp"
//
//    //得到时间戳
