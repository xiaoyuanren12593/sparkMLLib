...
/**
  ...
  */
trait ChaInsureinfoUntil extends Until with EnterpriseUntil {

  //渠道首次投保至今月数
  def ent_fist_plc_month(before: RDD[(String, String)], ods_ent_guzhu_salesman_channel: RDD[(String, String)],
                         sqlContext: HiveContext, en_before: collection.Map[String, String],
                         str: String): RDD[(String, String, String)] = {
    import sqlContext.implicits._

    //标签
    val ent_id = before.map(x => (x._1, x._2.split(";").filter(_.contains(str)).take(1).mkString("")))
    ...
    val end: RDD[(String, String, String)] = ods_ent_guzhu_salesman_channel
      ...
      .map(x => (en_before.getOrElse(x._1, "null"), s"${x._2.toInt}", str))
  ...
  /**
    * 渠道累加
    **/
  def channel_add(before: RDD[(String, String)], ods_ent_guzhu_salesman_channel: RDD[(String, String)],
                  sqlContext: HiveContext, en_before: collection.Map[String, String],
                  str: String): RDD[(String, String, String)] = {
    import sqlContext.implicits._

    //标签
    val ent_id = before.map(x => (x._1, x._2.split(";").filter(_.contains(str)).take(1).mkString(""))).filter(_._2.length > 1)
      .map(end => {
        val before = JSON.parseObject(end._2)
        //企业，和企业对应的数据
        (end._1, before.getString("value"), before.getString("qual"))
  ...
  //连续在保月份，都有哪个月
  def channel_add_month(before: RDD[(String, String)], ods_ent_guzhu_salesman_channel: RDD[(String, String)],
                        sqlContext: HiveContext, en_before: collection.Map[String, String],
                        str: String): RDD[(String, String, String)] = {
    import sqlContext.implicits._
    //标签
  ...
  //渠道月均保费
  def avg_month_premium(ent_summary_month_1: DataFrame, get_hbase_key_name: collection.Map[String, String],
                        sqlContext: HiveContext, ods_ent_guzhu_salesman_channel: RDD[(String, String)],
                        en_before: collection.Map[String, String], before: RDD[(String, String)],
                        str: String): RDD[(String, String, String)] = {
    import sqlContext.implicits._
    //标签
    val total_premium_data = before
      .map(x => (x._1, x._2.split(";").filter(_.contains(str)).take(1).mkString("")))
      .filter(_._2.length > 1)
      .map(end => {
        val before = JSON.parseObject(end._2)
    ...
    val tepTwo = ent_summary_month_1
      .filter("data_type='totalInsuredPersons'")
      .where("length(ent_id)>0")
      .select("ent_id")
      .map(x => (x.getString(0), 1))
      .reduceByKey(_ + _)
      .map(x => (x._1, x._2))
    val end_result = tepOne
      .join(tepTwo)
      .map(x => {
        val ent_id = x._1
    ...
    //根据企业名称进行join
    val end: RDD[(String, String, String)] = end_result
  ...
  //渠道投保人员占总人数比
  def insured_rate(ods_policy_detail: DataFrame, ods_policy_insured_detail: DataFrame,
                   ent_sum_level: DataFrame, get_hbase_key_name: collection.Map[String, String],
                   sqlContext: HiveContext, ods_ent_guzhu_salesman_channel: RDD[(String, String)],
    ...
    val ent_sum_level_data = ent_sum_level
      .select("ent_id", "ent_scale")
      .map(x => (x.getString(0), x.getString(1).toInt))
      .reduceByKey((x1, x2) => if (x1 >= x2) x1 else x2)

    val end_result = tepFour.join(ent_sum_level_data)
      .map(x => {
        val number_tb = x._2._1
        val number_total = x._2._2
  ...
  //渠道工种|赔付金额，累加比较
  def channel_add_type(before: RDD[(String, String)], ods_ent_guzhu_salesman_channel: RDD[(String, String)],
                       sqlContext: HiveContext, en_before: collection.Map[String, String],
                       str: String): RDD[(String, String, String)] = {
    import sqlContext.implicits._
    //标签
  ...
  //渠道月均在保人数
  def ent_month_plc_persons(ent_summary_month_1: DataFrame, get_hbase_key_name: collection.Map[String, String],
                            sqlContext: HiveContext, ods_ent_guzhu_salesman_channel: RDD[(String, String)],
                            en_before: collection.Map[String, String]): RDD[(String, String, String)] = {
    import sqlContext.implicits._

    val end_result = ent_summary_month_1
      .filter("data_type='totalInsuredPersons'")
    ...
    //根据企业名称进行join
    val end = end_result.join(ods_ent_guzhu_salesman_channel
      .toDF("channel_name", "ent_name"), "ent_name")
      .map(x => {
        val channel_name = x.getAs[String]("channel_name")
  ...
  //月均增减员次数
  def ent_month_regulation_times(ent_add_regulation_times_data: RDD[(String, String, String)],
                                 ent_summary_month_1: DataFrame, get_hbase_key_name: collection.Map[String, String],
                                 sqlContext: HiveContext, ods_ent_guzhu_salesman_channel: RDD[(String, String)],
                                 en_before: collection.Map[String, String]): RDD[(String, String, String)] = {
    import sqlContext.implicits._
    ...
    val month_number = total
      ...
      .filter(_._2 > 0)
    val end_result = ent_add_regulation_times_data
      .map(x => {
        (x._1, x._2)
      })
      .join(month_number)
