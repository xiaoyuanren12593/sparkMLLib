package bzn.label.channel_baseinfo
...
/**
  ...
  * baseinfo的实现方法
  */
trait ChaBaseinfoUntil extends  Until{
  //得到投保人投保了多少年(麻烦)
  ...
  = {
    ...
    //该企业中男生，女生的数量
    val man_woman_Percent_AgeAvg = join_qy_mx.map(x =>
    ...
    //根据企业名称进行join
    val man_woman_Percent_AgeAvg_end: RDD[(String, String, String)] = man_woman_Percent_AgeAvg
    ...
    man_woman_Percent_AgeAvg_end
  }


  //渠道产品ID
  def getqCp(en_before: collection.Map[String, String],
  ...
  = {
    ...
    val ent_id = before.map(x =>
      (x._1, x._2.split(";").filter(_.contains("ent_insure_code")).take(1).mkString(""))
    ...
    val qCp: RDD[(String, String, String)] = ods_ent_guzhu_salesman_channel.toDF("channel_name", "ent_name")

      .join(ent_id, "ent_name")
      .map(x => (x.getAs[String]("channel_name"), x.getAs[String]("value"))).reduceByKey((x1, x2) => x1 + "|" + x2).map(x =>
      (en_before.getOrElse(x._1, "null"), if (x._2.contains("|")) x._2.split("\\|").distinct.mkString("|") else x._2, "ent_insure_code")
    )
    qCp
  ...
  //渠道人员规模
  def qy_gm(en_before: collection.Map[String, String],
            before: RDD[(String, String)],
            usersRDD: RDD[String],
            channel_ent_name: Array[String],
            ods_ent_guzhu_salesman_channel: RDD[(String, String)],
  ...
  = {
    ...
    //标签
    val ent_id = before.map(x =>
      (x._1, x._2.split(";").filter(_.contains("ent_scale")).take(1).mkString(""))
    ).filter(_._2.length > 1)
      .map(end => {
        val before = JSON.parseObject(end._2)
        //企业，和企业对应的数据
    ...
    val qy_gm_r: RDD[(String, String, String)] = ods_ent_guzhu_salesman_channel.toDF("channel_name", "ent_name").join(ent_id, "ent_name")
      .map(x => (x.getAs[String]("channel_name"), x.getAs[String]("value").toInt))
      .reduceByKey((x1, x2) => x1 + x2)
      .map(x =>
        (en_before.getOrElse(x._1, "null"), x._2.toInt.toString, "ent_scale")
      )
    qy_gm_r
  ...
  //渠道潜在人员规模
  def qy_qz(en_before: collection.Map[String, String],
            before: RDD[(String, String)],
            usersRDD: RDD[String],
            channel_ent_name: Array[String],
            ods_ent_guzhu_salesman_channel: RDD[(String, String)],
  ...
  = {
    ...
    //标签
    val ent_id = before.map(x =>
      (x._1, x._2.split(";").filter(_.contains("ent_potential_scale")).take(1).mkString(""))
    ).filter(_._2.length > 1)
      .map(end => {
        val before = JSON.parseObject(end._2)
        //企业，和企业对应的数据
    ...
    val qy_qz_r: RDD[(String, String, String)] = ods_ent_guzhu_salesman_channel.toDF("channel_name", "ent_name").join(ent_id, "ent_name")
      .map(x => (x.getAs[String]("channel_name"), x.getAs[String]("value").toInt))
      .reduceByKey((x1, x2) => x1 + x2)
      .map(x =>
        (en_before.getOrElse(x._1, "null"), x._2.toInt.toString, "ent_potential_scale")
      )
    qy_qz_r
