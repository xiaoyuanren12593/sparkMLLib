...
trait ChaClaiminfoUntilTest extends Until {
  /**
    * 渠道风险等级
    **/
  def ent_work_risk_r_to_hbase(conf: Configuration, conf_fs: Configuration, tableName: String,
                               column_family: String, channel_before: RDD[(String, String)],
                               before: RDD[(String, String)], sql_context: HiveContext,
    ...
    def while_value(work_type: String): RDD[(String, String, String)] = {
      //标签
      val ent_id = before
        .map(x => (x._1, x._2.split(";").filter(_.contains(work_type)).take(1).mkString("")))
  ...
  def ent_monthly_risk(employer_liability_claims_r: DataFrame, ods_policy_detail_r: DataFrame,
                       ods_policy_insured_detail_r: DataFrame, get_hbase_key_name: collection.Map[String, String],
    ...
    val ods_policy_detail_before = ods_policy_detail_r
    ...
    //根据企业名称进行join
    val end: RDD[(String, String, String)] = result
  ...
  /**
    ...
    **/
  def ent_material_integrity(employer_liability_claims_r: DataFrame, ods_policy_detail_r: DataFrame,
  ...
  /**
    * 渠道平均赔付时效
    **/
  def avg_aging_cps(employer_liability_claims: DataFrame, ods_policy_detail: DataFrame,
                    get_hbase_key_name: collection.Map[String, String], sqlContext: HiveContext,
                    ods_ent_guzhu_salesman_channel: RDD[(String, String)],
                    en_before: collection.Map[String, String]): RDD[(String, String, String)] = {
    import sqlContext.implicits._
  ...
  def channel_add(before: RDD[(String, String)], ods_ent_guzhu_salesman_channel: RDD[(String, String)],
    ...
    //标签
    val ent_id = before
      .map(x => (x._1, x._2.split(";").filter(_.contains(str)).take(1).mkString(""))).filter(_._2.length > 1)
      .map(end => {
        //企业，和企业对应的数据
        val before = JSON.parseObject(end._2)
        (end._1, before.getString("value"), before.getString("qual"))
  ...
  /**
    * 渠道平均出险周期
    **/
  def avg_aging_risk(ods_policy_detail: DataFrame, ods_policy_risk_period: DataFrame,
                     get_hbase_key_name: collection.Map[String, String], sqlContext: HiveContext,
                     ods_ent_guzhu_salesman_channel: RDD[(String, String)],
                     en_before: collection.Map[String, String]): RDD[(String, String, String)] = {
    ...
    val tepThree = tepOne
      .join(tepTwo, ods_policy_detail("policy_code") === ods_policy_risk_period("policy_no"))
      .filter("length(ent_id)>0")
    val end_result = tepThree
  ...
  /**
    ...
    **/
  def mix_period_rate(ods_policy_detail: DataFrame, ods_policy_risk_period: DataFrame,
                      get_hbase_key_name: collection.Map[String, String], sqlContext: HiveContext,
                      ods_ent_guzhu_salesman_channel: RDD[(String, String)],
                      en_before: collection.Map[String, String]): RDD[(String, String, String)] = {
    import sqlContext.implicits._
    ...
    //总共有多少个
    val sum_Number = total
      .map(x => (x._1, 1))
      .reduceByKey(_ + _)
      .map(x => (x._1, x._2))
    val end_result = threeNumber
      .join(sum_Number)
      .map(x => (x._1, x._2._1, x._2._2))
      .filter(x => if (x._2 > 1) true else false)
      //概率
      .map(x => (s"${get_hbase_key_name.getOrElse(x._1, "null")}", x._2.toString, x._3.toString))
  ...
  def largecase_rate(ods_policy_detail: DataFrame, employer_liability_claims: DataFrame,
                     get_hbase_key_name: collection.Map[String, String], sql_context: HiveContext,
                     ods_ent_guzhu_salesman_channel: RDD[(String, String)],
                     en_before: collection.Map[String, String]): RDD[(String, String, String)] = {
    import sql_context.implicits._
