...
/**
  ...
  */
trait BaseinfoUntilTest extends Until with EnterpriseUntil {
  //得到投保人投保了多少年(麻烦)
  ...
  def qy_sex(ods_policy_insured_detail: DataFrame,
    ...
    val ods_policy_insured_detail_table = ods_policy_insured_detail
    ...
    val man_woman_Percent_AgeAvg: RDD[(String, String, String)] = join_qy_mx
  ...
  def qy_gm(ent_sum_level: DataFrame): RDD[(String, String, String)] = {
    ...
    val end = ent_sum_level
      .where("length(ent_id)>0")
      .select("ent_id", "ent_scale")
      .map(x => (x.getString(0), x.getString(1), "ent_scale"))
  ...
  def qy_pp(ent_sum_level: DataFrame): RDD[(String, String, String)] = {
    ...
    val end = ent_sum_level
      .where("length(ent_id)>0")
      .select("ent_id", "ent_influence_level")
      .map(x => (x.getString(0), x.getString(1), "ent_influence_level"))

    end
  }

  //企业潜在人员规模
  def qy_qz(ods_policy_detail: DataFrame, ods_policy_insured_detail: DataFrame,
            ent_sum_level: DataFrame): RDD[(String, String, String)] = {
    ...
    val before = ods_policy_detail
      .where("length(ent_id)>0 and policy_status in('0','1', '7', '9', '10') ")
      .select("ent_id", "policy_id", "policy_status")
    //得到被保人清单中的,id，和证件号,被保人投保状态，1为在保2为已减员3为失效
    val after = ods_policy_insured_detail
      .where("insure_policy_status='1'")
      .selectExpr("policy_id", "insured_cert_no as cur_insured_persons", "insure_policy_status")
    ...
    val esl = ent_sum_level
      .select("ent_id", "ent_scale")
      .filter("length(ent_id)>0 and  length(ent_scale)<10")
    ...
    val ents = end
      ...
      .map(x => (x._1, x._2 + "", "ent_potential_scale"))

    ents
  }

  //企业类型
  def ent_type(ent_enterprise_info: DataFrame, d_id_certificate: DataFrame,
               ods_policy_detail_table: DataFrame): RDD[(String, String, String)] = {
