...
trait InsureinfoUntilTest extends Until with EnterpriseUntil {
  ...
  def ent_month_regulation_times(ent_add_regulation_times_data: RDD[(String, String, String)], ent_summary_month_1: DataFrame): RDD[(String, String, String)] = {
    ...
    val month_number = total.map(x => (x.getString(0), x.getString(1)))
      ...
      .map(x => (x._1, x._2.split("\t").length))
      .filter(_._2 > 0)

    val end: RDD[(String, String, String)] = ent_add_regulation_times_data.map(x => (x._1, x._2))
      .join(month_number)
      .map(x => {
        //企业ID，累计增减员次数，月份ID次数
        val ent_id = x._1
  ...
  def ent_first_craft(ods_policy_insured_detail: DataFrame, ods_policy_detail: DataFrame, d_work_level: DataFrame): RDD[(String, String, String)] = {
    ...
    val result = ss
      .join(tepThree, ss("insured_work_type") === tepThree("work_type"))
    ...
    //求出在该企业第一工种出现最多的次数
    val end = result.map(x => ((x.getString(4), x.getString(2)), 1))
      .reduceByKey(_ + _)
  ...
  def ent_second_craft(ods_policy_insured_detail: DataFrame, ods_policy_detail: DataFrame, d_work_level: DataFrame): RDD[(String, String, String)] = {
    ...
    //求出在该企业第二工种出现最多的次数
    val end = result
      .map(x => ((x.getString(4), x.getString(2)), 1))
      .reduceByKey(_ + _)
      .map(x => (x._1._1, (x._1._2, x._2)))
      .groupByKey
      .flatMap(x => {
  ...
  def ent_most_money_craft(ods_policy_insured_detail: DataFrame, ods_policy_detail: DataFrame, d_work_level: DataFrame, employer_liability_claims: DataFrame): RDD[(String, String, String)] = {
    ...
    val end: RDD[(String, String, String)] = tepFive
      .select("ent_id", "work_type", "final_payment")
      .map(x => (x.getString(0), (x.getString(1), x.get(2).toString.toDouble)))
      .reduceByKey((x1, x2) => {
  ...
  def ent_most_count_craft(ods_policy_insured_detail: DataFrame, ods_policy_detail: DataFrame, employer_liability_claims: DataFrame): RDD[(String, String, String)] = {
    ...
    //找出哪种类型的工种出险最多
    val end: RDD[(String, String, String)] = tepFive
      .select("ent_id", "insured_work_type")
      .map(x => ((x.getString(0), x.getString(1)), 1))
      .reduceByKey(_ + _)
  ...
  def insured_rate(ods_policy_detail: DataFrame, ods_policy_insured_detail: DataFrame, ent_sum_level: DataFrame): RDD[(String, String, String)] = {
    ...
    val tepFour = tepThree.map(x => (x.getString(1), x.getString(2)))
      .reduceByKey((x1, x2) => {
        val res = x1 + "\t" + x2
        res
      })
  ...
  def effective_policy(ods_policy_detail: DataFrame): RDD[(String, String, String)] = {
    ...
    val end: RDD[(String, String, String)] = tepOne
      ...
      .reduceByKey(_ + _)
      .map(x => (x._1, x._2 + "", "effective_policy"))
  ...
  def total_insured_count(ods_policy_detail: DataFrame, ods_policy_insured_detail: DataFrame): RDD[(String, String, String)] = {
    ...
    val end: RDD[(String, String, String)] = tepThree
      .map(x => (x.getString(1), 1))
      .reduceByKey(_ + _)
      .map(x => (x._1, x._2 + "", "total_insured_count"))
  ...
  def total_insured_persons(ods_policy_detail: DataFrame, ods_policy_insured_detail: DataFrame): RDD[(String, String, String)] = {
    ...
    val end: RDD[(String, String, String)] = tepThree
      .map(x => (x.getString(1), x.getString(3)))
      .reduceByKey((x1, x2) => {
        val res = x1 + "\t" + x2
        res
      })
      .map(x => (x._1, x._2.split("\t").distinct.length + "", "total_insured_persons"))
  ...
  def cur_insured_persons(ods_policy_detail: DataFrame, ods_policy_insured_detail: DataFrame): RDD[(String, String, String)] = {
    ...
    val end: RDD[(String, String, String)] = tepThree
      ...
      .map(x => (x._1, x._2.split("\t").distinct.length + "", "cur_insured_persons"))
...
//      .where("dim_1 in ('外包雇主','骑士保','大货车')")
    ...
    }).filter(x => x._3 == "重庆翔耀保险咨询服务有限公司").toDF("ent_id","ent_name","channel_name")
    ...
    val ods_policy_detail = ods_policy_detail_temp.join(dim_1,ods_policy_detail_temp("insure_code") ===  dim_1("product_code"))
      ...
      }).filter(x => x._1 != null).toDF("holder_company","policy_code","policy_id")
...
//      .filter("ent_id = 'd18de1e66c4b39d92fe21db364372df9'")
  ...
  def total_premium(ods_policy_detail: DataFrame): RDD[(String, String, String)] = {
    ...
    val tepOne = ods_policy_detail
      .filter("policy_status in('0','1', '7', '9', '10') and ent_id!=''")
      .select("ent_id", "premium")
    //    |              ent_id|    premium|
    //    |0a789d56b7444d519...| 62905.1200|
    val end: RDD[(String, String, String)] = tepOne.map(x => (x.getString(0), x.get(1).toString.toDouble))
      .reduceByKey(_ + _)
      // (x._1, numberFormat.format(x._2), "total_premium")
      .map(x => (x._1, x._2.toString, "total_premium"))
    end
  }
  ...
  def avg_month_premium(total_premium_data: RDD[(String, String, String)], ent_summary_month_1: DataFrame): RDD[(String, String, String)] = {
    ...
    val tepTwo = ent_summary_month_1
      ...
      .select("ent_id").map(x => (x.getString(0), 1))
      .reduceByKey(_ + _)
      .map(x => (x._1, x._2))
    val end: RDD[(String, String, String)] = tepOne
      .join(tepTwo)
  ...
  def ent_continuous_plc_month(ent_summary_month_1: DataFrame): RDD[(String, String, String)] = {
    ...
    val end: RDD[(String, String, String)] = ent_summary_month_1
      .filter("data_type='totalInsuredPersons'")
      .select("month_id", "data_val", "ent_id")
