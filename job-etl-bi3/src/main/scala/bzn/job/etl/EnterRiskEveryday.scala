...
object EnterRiskEveryday extends Until {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val dim_product = sqlContext.sql("select x.product_code from odsdb_prd.dim_product x where x.product_type_2='蓝领外包'")
      .map(_.getAs[String]("product_code"))
      .collect()
    ...
    //企业价值：得到标签数据
    val vectors = getHbase_label(tepOne, d_city_grade_map)
      .filter(x => if (x._2.length > 0 && ods_bro_end.value.contains(x._2)) true else false)
      .map(x => (x._2, Vectors.dense(x._1), x._1(1)))
