...
/**
  ...
  * 雇主预警
  */
object EarlyWarningTest extends early_until {
  //遍历某目录下所有的文件和子文件
...
//    resTemp.foreach(println)
    ...
    val end_result: RDD[String] = add_person.union(subtraction_person).union(guarantee_person).union(customer_loss).union(test)
    end_result
