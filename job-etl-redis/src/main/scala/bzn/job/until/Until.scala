...
trait Until {
  ...
  : Array[(String, Int, String, String, Integer, String, String, String, String, String, String, String, String)] = {
    ...
    val result: Array[String] = result_str
      .split(" ")
      .map(x => {
  ...
  //使用知识库并获得答案返回
  def getAnswer(tep_Twos: DataFrame, result_str: String, stop: StopRecognition, end_case
  : Array[(String, Int, String, String, Integer, String, String, String, String, String, String, String, String)]): JSONArray = {

    val response_Json_array: JSONArray = new JSONArray()
    //排序:只取得前6个
    val result_order = end_case.sortBy(x => x._4.toDouble).reverse.take(6)
  ...
  /**
    ...
    **/
  def get_Node_ID(result_str: String, stop: StopRecognition, session: Session, category: String)
  : (Array[(String, String, String)], Array[String]) = {
    ...
    //将中文为一个字的过滤掉
    val result_str_two_chinese: Array[String] = result_str
      .split(" ")
      .map(x => {
        val char_size = x.getBytes("UTF-8").length
        (x, char_size)
      ...
      .map(_._1)
    //找到我历史数据中得节点值
    val res: Array[(String, String, String)] = get_Labels_Array
      .map(x => {
  ...
  def get_id(tep_one: Array[String], session: Session): Array[(String, String)] = {
    ...
    val end: Array[(String, String)] = json_arr
      .toArray()
      .map(x => JSON.parseObject(x.toString))
      .map(x => (x.getString("node_name"), x.getString("id")))
      .filter(x => tep_one.contains(x._1))
  ...
  /**
    ...
    **/
  def get_relationShip_name_id(res: Array[(String, String, String)],
                               result_str: String, session: Session): Array[(String, String, String)] = {
    //得到我每个节点的id值
  ...
  /**
    ...
    **/
  def filter_relationShips(stop: StopRecognition, result_str_two_chinese: Array[String],
                           last_one: Array[(String, String, String)], session: Session): Array[(String, String, String)] = {
    ...
    //将我的问题中的关系过滤出来
    val relaction_filter = relation_ships_name_arr
      .distinct
      .map(x => {
        val value = DicAnalysis.parse(x).recognition(stop).toStringWithOutNature(" ").split(" ")
        (x, value)
      })
      .filter(line => line._2.exists(result_str_two_chinese.contains(_)))
      .map(_._1)
  ...
  /**
    * 调用知识图谱
    **/
  def get_knowledge_graph(result_str: String, stop: StopRecognition,
                          session: Session, category: String): Array[(String, String, String)] = {
  ...
  //工种得到预测的结果完全能对的上的
  def getProduct_result_work_all(robot_data_json_before: Array[String],
                                 str: String): Array[((String, String), Int)] = {
    val robot_data_json = robot_data_json_before.filter(x => {
    ...
    val result_value: Array[((String, String), Int)] = robot_data_json
      .map(x => {
        val json_res = JSON.parseObject(x)
  ...
  //工种得到预测的结果案情
  def getProduct_result_work(robot_data_json_before: Array[String], result_str: String,
                             stop: StopRecognition): Array[(String, Int, String, String, String, String, String)] = {
    //鉴定是否为空
    def if_not_null(str: String): String = {
      if (str == null || str == "null" || str == "") "" else str
    }
    ...
    //找到我历史数据中得值
    val sentenceData = robot_data_json.map(x => JSON.parseObject(x).getString("worktype_name"))
    val res: Set[String] = sentenceData.flatMap(x => DicAnalysis.parse(x).recognition(stop).toStringWithOutNature(" ").split(" ")).toSet
  ...
  //工种对应表
  def getProduct_result_work_mapping(robot_data_json_before: Array[String],
                                     result_str: String, stop: StopRecognition): Array[(String, Int, String, String)] = {
    //鉴定是否为空
    def if_not_null(str: String): String = {
      if (str == null || str == "null" || str == "") "" else str
    }
    ...
    //找到我历史数据中得值
    val sentenceData = robot_data_json.map(x => JSON.parseObject(x).getString("work_type"))
    val res: Set[String] = sentenceData.flatMap(x => DicAnalysis.parse(x).recognition(stop).toStringWithOutNature(" ").split(" ")).toSet
  ...
  //工种使用知识库并获得答案返回
  def getAnswer_work(stop: StopRecognition, just_one: String,
                     end_case_work_mapping: Array[(String, Int, String, String)]): JSONArray = {
    val response_Json_array: JSONArray = new JSONArray()
    //排序:只取得前6个
