      ...
      if (!rdds.isEmpty()) {
        ...
        val vectors = rdds
          .map(x => {
            ...
            (risk_level_features, entValueScore, entRiskLevel, insuredRiskLevel, policyPayRate, requestId,
              policyId, entId, insuredCertNo, injury)
          })
