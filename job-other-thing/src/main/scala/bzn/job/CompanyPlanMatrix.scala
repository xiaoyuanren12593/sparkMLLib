...
//      .setMaster("local[*]")
    ...
    /**
      * 读取产品表
      */
    val dimProduct = sQLContext.read.jdbc(url,"dim_product",prop)
    ...
    /**
      * 保单产品
      */
    val insuredProductRes = planInsuredPolicyRes.join(dimProduct,planInsuredPolicyRes("insure_code")===dimProduct("product_code"))
