...
object PersonalClaimServiceTest extends Until with PersonalUntil {
  ...
  def claim_count(employer_liability_claims: DataFrame): RDD[(String, String, String)] = {
    ...
    val end = employer_liability_claims
      .filter("length(cert_no)>2 and length(paper_finish_date)>0")
      .select("cert_no")
      .map(x => (x.getString(0), 1))
      .reduceByKey(_ + _)
      .map(x => (x._1, x._2 + "", "claim_count"))
  ...
  def death_count(employer_liability_claims: DataFrame): RDD[(String, String, String)] = {
    val end = employer_liability_claims
      .filter("length(cert_no)>2 and case_type='死亡'")
      .select("cert_no")
      .map(x => (x.getString(0), 1))
      .reduceByKey(_ + _)
      .map(x => (x._1, x._2 + "", "death_count"))
  ...
  def disability_count(employer_liability_claims: DataFrame): RDD[(String, String, String)] = {
    val end: RDD[(String, String, String)] = employer_liability_claims
      ...
      .map(x => (x.getString(0), 1))
      .reduceByKey(_ + _)
      .map(x => (x._1, x._2 + "", "disability_count"))
  ...
  def case_work_count(employer_liability_claims: DataFrame): RDD[(String, String, String)] = {
    val end: RDD[(String, String, String)] = employer_liability_claims
      ...
      .map(x => (x.getString(0), 1))
      .reduceByKey(_ + _)
      .map(x => (x._1, x._2 + "", "case_work_count"))
  ...
  def case_notwork_count(employer_liability_claims: DataFrame): RDD[(String, String, String)] = {
    val end = employer_liability_claims
      ...
      .map(x => (x.getString(0), 1))
      .reduceByKey(_ + _)
      .map(x => (x._1, x._2 + "", "case_notwork_count"))
  ...
  def prepay_total(employer_liability_claims: DataFrame): RDD[(String, String, String)] = {
    val end = employer_liability_claims
      .filter("length(cert_no)> 14")
      .filter("pre_com is not null")
      .select("cert_no", "pre_com")
      .map(x => x)
      .filter(x => {
      ...
      .map(x => (x.getString(0).trim(), x.get(1).toString))
      .filter(_._2 != ".")
      .filter(_._2 != "#N/A")
      .map(x => (x._1, x._2.toDouble))
      .reduceByKey(_ + _)
      .map(x => (x._1, x._2.toInt + "", "prepay_total"))
  ...
  def prepay_death(employer_liability_claims: DataFrame): RDD[(String, String, String)] = {
    val end = employer_liability_claims
      ...
      .map(x => (x.getString(0), x.get(1).toString))
      .filter(_._2 != ".")
      .filter(_._2 != "#N/A")
      .map(x => (x._1, x._2.toDouble))
      .reduceByKey(_ + _)
      .map(x => (x._1, x._2.toInt + "", "prepay_death"))
  ...
  def prepay_disability(employer_liability_claims: DataFrame): RDD[(String, String, String)] = {
    val end = employer_liability_claims
      ...
      .map(x => (x._1, x._2.toDouble))
      .reduceByKey(_ + _)
      .map(x => (x._1, x._2.toInt + "", "prepay_disability"))
  ...
  def prepay_work(employer_liability_claims: DataFrame): RDD[(String, String, String)] = {
    val end = employer_liability_claims
      ...
      .map(x => (x._1, x._2.toDouble))
      .reduceByKey(_ + _)
      .map(x => (x._1, x._2.toInt + "", "prepay_work"))
  ...
  def prepay_notwork(employer_liability_claims: DataFrame): RDD[(String, String, String)] = {
    val end = employer_liability_claims
      ...
      .map(x => (x._1, x._2.toDouble))
      .reduceByKey(_ + _)
      .map(x => (x._1, x._2.toInt + "", "prepay_notwork"))
  ...
  def avg_finalpay(employer_liability_claims: DataFrame): RDD[(String, String, String)] = {
    ...
    val num_Y = end
      ...
      .reduceByKey(_ + _)
      .map(x => (x._1, x._2.toDouble))
    val ends: RDD[(String, String, String)] = yG
      .join(num_Y)
      .map(x => {
        val yG = x._2._1
        val num_total = x._2._2
  ...
  def case_overtime_count(employer_liability_claims: DataFrame): RDD[(String, String, String)] = {
    val end = employer_liability_claims
      .filter("overtime='是'")
      .where("length(cert_no)>2")
      .select("cert_no")
      .map(x => (x.getString(0), 1))
      .reduceByKey(_ + _)
      .map(x => (x._1, x._2 + "", "case_overtime_count"))
  ...
  def main(args: Array[String]): Unit = {
    ...
    report_count_s.take(10).foreach(x => println(x))
    ...
    claim_count_rs.take(10).foreach(x => println(x))
    ...
    death_count_s.take(10).foreach(x => println(x))
    ...
    disability_count_s.take(10).foreach(x => println(x))
    ...
    case_work_count_s.take(10).foreach(x => println(x))
    ...
    case_notwork_count_s.take(10).foreach(x => println(x))
    ...
    prepay_total_s.take(10).foreach(x => println(x))
    ...
    prepay_death_s.take(10).foreach(x => println(x))
    ...
    prepay_disability_s.take(10).foreach(x => println(x))
    ...
    prepay_work_s.take(10).foreach(x => println(x))
    ...
    prepay_notwork_s.take(10).foreach(x => println(x))
    ...
    finalpay_total_s.take(10).foreach(x => println(x))
    ...
    avg_prepay_s.take(10).foreach(x => println(x))
    ...
    avg_finalpay_s.take(10).foreach(x => println(x))
    ...
    case_overtime_count_s.take(10).foreach(x => println(x))
    ...
    avg_effecttime_s.take(10).foreach(x => println(x))
  }
