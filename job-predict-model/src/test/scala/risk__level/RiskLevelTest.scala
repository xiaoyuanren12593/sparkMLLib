...
/**
  ...
  */
object RiskLevelTest extends risl_until {
  //找到最优的分类
  ...
  def main(args: Array[String]): Unit = {
    ...
    val employer_liability_claims = sqlContext.sql("select * from odsdb_prd.employer_liability_claims")
      .filter("length(risk_date)>3 and length(report_date)>3").cache
    val ods_policy_insured_detail = sqlContext.sql("select * from odsdb_prd.ods_policy_insured_detail")
      .filter("length(insured_start_date)>3").select("insured_cert_no", "insured_start_date")
    ...
    val result = ods_policy_insured_detail.join(tep_One, "insured_cert_no").map(x => {
      ...
      .filter(x => {
        var str = ""
        if( x._2._6 != null){
          str = x._2._6.toString
        }
        val p = Pattern.compile("[\u4e00-\u9fa5]")
        val m = p.matcher(str)
        if (!m.find ) true else false
      })
      .filter(x => x._2._6 != null)
      .filter(x => x._2._6 != "#N/A")
      .map(x => {
    ...
    scaledData.take(100).foreach(println)
