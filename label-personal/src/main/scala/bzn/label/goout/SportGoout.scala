package bzn.label.goout

import bzn.common.Until
import org.apache.spark.sql.hive.HiveContext
import org.apache.spark.{SparkConf, SparkContext}

object SportGoout extends Until {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val total = dim_product.filter("product_type_1 = '体育'").select("product_code", "product_name").map(x => {
    ...
    //马拉松
    val mL = total.filter(_._8 != "-1").map(x => (x._1, x._8))
      .union(total.filter(_._9 != "-1").map(x => (x._1, x._9))).map(x => (x._1, (x._2, "user_marathon")))
    ...
    //找到保单号，保单ID，和每个人的身份证
    val tepThree = ods_policy_detail.where("policy_status in ('0','1','7','9','10')").select("ent_id", "policy_id", "insure_code")
    val tepFour = ods_policy_insured_detail.filter("length(insured_cert_no)=18 and length(insured_create_time)>14").select("policy_id", "insured_cert_no", "insured_create_time")
