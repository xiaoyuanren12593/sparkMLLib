...
import company.hbase_label.channel.channel_trait.cha_Claiminfo_until
...
/**
  ...
  */
object Cha_claiminfo extends cha_Claiminfo_until with until {
  ...
  /**
    * 渠道报案时效（小时）
    **/
  def ent_report_time(employer_liability_claims_r: DataFrame,
  ...
  = {
    ...
    val ods_policy_detail = ods_policy_detail_r.select("ent_id", "policy_code").map(x => (x.getString(1), x.getString(0)))
    //根据保单ID做一个Join，找到我对应的在保单ID中的数据(企业ent_id在保单表中)
    //ent_id，小时(存到了hbase中的ent_report_time)
    val end_result = employer_liability_claims.join(ods_policy_detail).map(x => {
  ...
  /**
    * 渠道平均申请理赔周期(天)
    **/
  def avg_aging_claim(employer_liability_claims: DataFrame,
  ...
  = {
    ...
    val tep_two = ods_policy_detail.filter("LENGTH(ent_id)>0").select("policy_code", "ent_id").map(x => (x.getString(0), x.getString(1)))
    val end_result = tep_one.join(tep_two).map(x =>
      (x._2._2, (x._2._1.toDouble, 1)) //ent_id | 每个人补齐的天数 |1
    ).reduceByKey((x1, x2) => {
  ...
  def Claminfo(usersRDD: RDD[String],
  ...
  = {
    ...
    val columnFamily1: String = "claiminfo"
    ...
    //渠道风险等级 ，里面有toHbase
    ent_work_risk_r_To_Hbase(conf, conf_fs, tableName, columnFamily1, channel_before, before, sqlContext, ods_ent_guzhu_salesman_channel, en_before)

    //渠道增减行为(人数):在同一个年单中超过2次减员的人的个数
    val ent_employee_increase_r = channel_add(before, ods_ent_guzhu_salesman_channel, sqlContext, en_before, "ent_employee_increase").filter(_._1.length > 5)
    toHbase(ent_employee_increase_r, columnFamily1, "ent_employee_increase", conf_fs, tableName, conf)

    //渠道出险概率,每百人月均出险概率
    val ent_monthly_risk_r = ent_monthly_risk(employer_liability_claims, ods_policy_detail, ods_policy_insured_detail, get_hbase_key_name, sqlContext, ods_ent_guzhu_salesman_channel, en_before).filter(_._1.length > 5)
    toHbase(ent_monthly_risk_r, columnFamily1, "ent_monthly_risk", conf_fs, tableName, conf)


    //渠道材料完整度
    val ent_material_integrity_r = ent_material_integrity(employer_liability_claims, ods_policy_detail, get_hbase_key_name, sqlContext, ods_ent_guzhu_salesman_channel, en_before).filter(_._1.length > 5)
    toHbase(ent_material_integrity_r, columnFamily1, "ent_material_integrity", conf_fs, tableName, conf)


    //渠道报案时效（小时）
    val ent_report_time_r = ent_report_time(employer_liability_claims, ods_policy_detail, get_hbase_key_name, sqlContext, ods_ent_guzhu_salesman_channel, en_before).filter(_._1.length > 5)
    toHbase(ent_report_time_r, columnFamily1, "ent_report_time", conf_fs, tableName, conf)


    //渠道平均申请理赔周期(天)
    val avg_aging_claim_r = avg_aging_claim(employer_liability_claims, ods_policy_detail, get_hbase_key_name, sqlContext, ods_ent_guzhu_salesman_channel, en_before).filter(_._1.length > 5)
    toHbase(avg_aging_claim_r, columnFamily1, "avg_aging_claim", conf_fs, tableName, conf)


    //渠道平均赔付时效
    val avg_aging_cps_r = avg_aging_cps(employer_liability_claims, ods_policy_detail, get_hbase_key_name, sqlContext, ods_ent_guzhu_salesman_channel, en_before).filter(_._1.length > 5)
    toHbase(avg_aging_cps_r, columnFamily1, "avg_aging_cps", conf_fs, tableName, conf)


    //渠道报案件数
    val report_num_r = channel_add(before, ods_ent_guzhu_salesman_channel, sqlContext, en_before, "report_num").filter(_._1.length > 5)
    toHbase(report_num_r, columnFamily1, "report_num", conf_fs, tableName, conf)

    //渠道理赔件数
    val claim_num_r = channel_add(before, ods_ent_guzhu_salesman_channel, sqlContext, en_before, "claim_num").filter(_._1.length > 5)
    toHbase(claim_num_r, columnFamily1, "claim_num", conf_fs, tableName, conf)


    //渠道死亡案件统计
    val death_num_r = channel_add(before, ods_ent_guzhu_salesman_channel, sqlContext, en_before, "death_num").filter(_._1.length > 5)
    toHbase(death_num_r, columnFamily1, "death_num", conf_fs, tableName, conf)


    //渠道伤残案件数
    val disability_num_r = channel_add(before, ods_ent_guzhu_salesman_channel, sqlContext, en_before, "disability_num").filter(_._1.length > 5)
    toHbase(disability_num_r, columnFamily1, "disability_num", conf_fs, tableName, conf)


    //渠道工作期间案件数
    val worktime_num_r = channel_add(before, ods_ent_guzhu_salesman_channel, sqlContext, en_before, "worktime_num").filter(_._1.length > 5)
    toHbase(worktime_num_r, columnFamily1, "worktime_num", conf_fs, tableName, conf)


    //渠道非工作期间案件数
    val nonworktime_num_r = channel_add(before, ods_ent_guzhu_salesman_channel, sqlContext, en_before, "nonworktime_num").filter(_._1.length > 5)
    toHbase(nonworktime_num_r, columnFamily1, "nonworktime_num", conf_fs, tableName, conf)


    //渠道预估总赔付金额
    val pre_all_compensation_r = channel_add(before, ods_ent_guzhu_salesman_channel, sqlContext, en_before, "pre_all_compensation").filter(_._1.length > 5)
    toHbase(pre_all_compensation_r, columnFamily1, "pre_all_compensation", conf_fs, tableName, conf)


    //渠道死亡预估配额
    val pre_death_compensation_r = channel_add(before, ods_ent_guzhu_salesman_channel, sqlContext, en_before, "pre_death_compensation").filter(_._1.length > 5)
    toHbase(pre_death_compensation_r, columnFamily1, "pre_death_compensation", conf_fs, tableName, conf)


    //渠道伤残预估配额
    val pre_dis_compensation_r = channel_add(before, ods_ent_guzhu_salesman_channel, sqlContext, en_before, "pre_dis_compensation").filter(_._1.length > 5)
    toHbase(pre_dis_compensation_r, columnFamily1, "pre_dis_compensation", conf_fs, tableName, conf)


    //渠道工作期间预估赔付
    val pre_wt_compensation_r = channel_add(before, ods_ent_guzhu_salesman_channel, sqlContext, en_before, "pre_wt_compensation").filter(_._1.length > 5)
    toHbase(pre_wt_compensation_r, columnFamily1, "pre_wt_compensation", conf_fs, tableName, conf)


    //渠道非工作期间预估赔付
    val pre_nwt_compensation_r = channel_add(before, ods_ent_guzhu_salesman_channel, sqlContext, en_before, "pre_nwt_compensation").filter(_._1.length > 5)
    toHbase(pre_nwt_compensation_r, columnFamily1, "pre_nwt_compensation", conf_fs, tableName, conf)


    //渠道实际已赔付金额
    val all_compensation_r = channel_add(before, ods_ent_guzhu_salesman_channel, sqlContext, en_before, "all_compensation").filter(_._1.length > 5)
    toHbase(all_compensation_r, columnFamily1, "all_compensation", conf_fs, tableName, conf)


    //渠道超时赔付案件数
    val overtime_compensation_r = channel_add(before, ods_ent_guzhu_salesman_channel, sqlContext, en_before, "overtime_compensation").filter(_._1.length > 5)
    toHbase(overtime_compensation_r, columnFamily1, "overtime_compensation", conf_fs, tableName, conf)

    //渠道已赚保费
    val charged_premium_r = channel_add(before, ods_ent_guzhu_salesman_channel, sqlContext, en_before, "charged_premium").filter(_._1.length > 5)
    toHbase(charged_premium_r, columnFamily1, "charged_premium", conf_fs, tableName, conf)

    //渠道拒赔次数
    val ent_rejected_count_r = channel_add(before, ods_ent_guzhu_salesman_channel, sqlContext, en_before, "ent_rejected_count").filter(_._1.length > 5)
    toHbase(ent_rejected_count_r, columnFamily1, "ent_rejected_count", conf_fs, tableName, conf)


    //渠道撤案次数
    val ent_withdrawn_count_r = channel_add(before, ods_ent_guzhu_salesman_channel, sqlContext, en_before, "ent_withdrawn_count").filter(_._1.length > 5)
    toHbase(ent_withdrawn_count_r, columnFamily1, "ent_withdrawn_count", conf_fs, tableName, conf)


    //渠道平均出险周期
    val avg_aging_risk_r = avg_aging_risk(ods_policy_detail, ods_policy_risk_period, get_hbase_key_name, sqlContext, ods_ent_guzhu_salesman_channel, en_before).filter(_._1.length > 5)
    toHbase(avg_aging_risk_r, columnFamily1, "avg_aging_risk", conf_fs, tableName, conf)


    //渠道极短周期个数（出险周期小于3的案件数）
    val mix_period_count_r = channel_add(before, ods_ent_guzhu_salesman_channel, sqlContext, en_before, "mix_period_count").filter(_._1.length > 5)
    toHbase(mix_period_count_r, columnFamily1, "mix_period_count", conf_fs, tableName, conf)


    //渠道极短周期百分比
    val mix_period_rate_r = mix_period_rate(ods_policy_detail, ods_policy_risk_period, get_hbase_key_name, sqlContext, ods_ent_guzhu_salesman_channel, en_before).filter(_._1.length > 5)
    toHbase(mix_period_rate_r, columnFamily1, "mix_period_rate", conf_fs, tableName, conf)


    //渠道重大案件率
    val largecase_rate_r = largecase_rate(ods_policy_detail, employer_liability_claims, get_hbase_key_name, sqlContext, ods_ent_guzhu_salesman_channel, en_before).filter(_._1.length > 5)
