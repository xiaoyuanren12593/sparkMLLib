...
//    conf.set(TableInputFormat.INPUT_TABLE, "labels:label_user_enterprise_vT")
    conf.set(TableInputFormat.INPUT_TABLE, "labels:label_channel_vT")
    //labels:label_channel_vT
...
//      val ends = if (tep_one.contains(ent_id)) "是雇主" else "非雇主"
     val b  =  baseinfo.concat(";").concat(insureinfo).concat(";").concat(claiminfo).concat(";")//.concat(ends)
//      (ent_name,b)
      b
    }}
//      .filter(x=>a.contains(x._1)).map(_._2).foreach(println(_))
     .repartition(1).saveAsTextFile("C:\\Users\\a2589\\Desktop\\enter_hbases1")
  }
}
