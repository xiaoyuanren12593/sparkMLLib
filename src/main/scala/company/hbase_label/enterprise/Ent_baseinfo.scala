...
import company.hbase_label.enterprise.enter_until.Baseinfo_until
...
//    val ods_policy_detail_table: DataFrame = sqlContext.sql("select policy_id,ent_id,user_id,policy_status from odsdb_prd.ods_policy_detail").cache()
    ...
    val rt = ent_enterprise_info.select("id", "create_time").map(x => (x.getString(0), x.getString(1).split(" ")(0), "register_time")).distinct()
    ...
    val en = ent_enterprise_info.select("id", "ent_name").filter("length(ent_name)>0").map(x => (x.getString(0), x.getString(1), "ent_name")).distinct()
    toHbase(en, columnFamily1, "ent_name", conf_fs, tableName, conf)

    //企业所在省份：ent_province [69d42cd85a59444895210bb781919228,湖南省]
    val ep = ent_enterprise_info.select("id", "office_province").where("id IS NOT NULL").join(d_cant.select("name", "code"), ent_enterprise_info("office_province") === d_cant("code")).map(x => (x.get(0) + "", x.get(2) + "", "ent_province")).distinct()
    toHbase(ep, columnFamily1, "ent_province", conf_fs, tableName, conf)

    //企业所在城市：ent_city [309925dae0cd41f78dd25fd93a30e004,邯郸市]
    val ec = ent_enterprise_info.select("id", "office_city").where("id IS NOT NULL").join(d_cant.select("short_name", "code"), ent_enterprise_info("office_city") === d_cant("code")).map(x => (x.get(0) + "", x.get(2) + "", "ent_city")).distinct()
    toHbase(ec, columnFamily1, "ent_city", conf_fs, tableName, conf)

    //该企业所在的城市类型：ent_city_type
    val ect = ent_enterprise_info.select("id", "office_city").join(d_city_grade.select("city_grade", "city_code"), ent_enterprise_info("office_city") === d_city_grade("city_code")).filter("office_city>1").filter("city_code>1").map(x => (x(0) + "", x(2) + "", "ent_city_type")).distinct()
