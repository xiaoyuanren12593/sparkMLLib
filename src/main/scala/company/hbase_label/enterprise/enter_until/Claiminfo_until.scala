...
trait Claiminfo_until {
  ...
  def ent_work_risk(ods_policy_insured_detail_r: DataFrame, ods_policy_detail_r: DataFrame, d_work_level_r: DataFrame): RDD[(String, String, String)] = {
    ...
    val result = ss.join(d_work_level, ss("insured_work_type") === d_work_level("work_type")).where("insure_policy_status='1' and ai_level is not null")
  ...
  def ent_employee_increase(ods_policy_preserve_detail: DataFrame, ods_policy_detail: DataFrame): RDD[(String, String, String)] = {
    ...
    val result: RDD[(String, String, String)] = jy.join(bd_id).map(x => {
      //     val polide_id =  x._1 //police_id
  ...
  def ent_monthly_risk(employer_liability_claims_r: DataFrame, ods_policy_detail_r: DataFrame, ods_policy_insured_detail_r: DataFrame): RDD[(String, String, String)] = {
    ...
    //    val employer_liability_claims = sqlContext.sql("select" +
    //      " lc.policy_no" +
    //      " from odsdb_prd.employer_liability_claims lc")
    val ods_policy_detail_before = ods_policy_detail_r.filter("length(ent_id)>0").select("ent_id", "policy_code")
    //    val ods_policy_detail_before = sqlContext.sql("select" +
    //      " pd.ent_id," +
    //      " pd.policy_code" +
    ...
    //      "pid.policy_id " +
    //      "from odsdb_prd.ods_policy_insured_detail pid")

    val ods_policy_detail = ods_policy_detail_r.select("policy_id", "ent_id", "policy_status")
    //    val ods_policy_detail = sqlContext.sql("select " +
    //      "pd1.policy_id, " +
    //      "pd1.ent_id ," +
    //      "pd1.policy_status " +
    //      "from odsdb_prd.ods_policy_detail pd1")
    val t2: RDD[(String, Int)] = ods_policy_insured_detail.join(ods_policy_detail, "policy_id")
      .where("policy_status in ('0','1', '7', '9', '10')")
      .filter("length(ent_id)>0")
      .map(x => {
        (x.getString(1), (x.getString(0), x.getString(2)))
      }).groupByKey.map(x => {
      //      x._1 ent_id
      //      x._2.size 计算次数
      (x._1, x._2.size)
    ...
    //ent_id,size
    //    (91370303MA3CF1N20E,69)

    //计算出险率：该企业出险的次数*100/该企业的投保人数
    val numberFormat = NumberFormat.getInstance
    numberFormat.setMaximumFractionDigits(4)
    //企业ID(rowKey),出险率(存到了Hbase中的ent_monthly_risk)
    val result: RDD[(String, String, String)] = t1.join(t2).filter(_._1 != "").map(x => {
      //ent_id
      val ed: String = x._1
      ...
      //      (ed, res.toString, "ent_monthly_risk")
  ...
  def ent_material_integrity(employer_liability_claims_r: DataFrame, ods_policy_detail_r: DataFrame): RDD[(String, String, String)] = {
    ...
    val tep_two = tep_one.map(x => {
      //ent_id | policy_no
      (x.getString(2), x.getString(0))
    }).groupByKey.map(x => {
      (x._1, x._2.size)
    })

    //计算我这个企业中发生意外险的这张保单号，是否补过资料 (因为它补过资料所以是不完整的，最后的结果是用1-不完整度就是完整度了)
    val tep_three = tep_one.filter("if_resubmit_paper='是'").map(x => {
      //ent_id      |   保单号
      (x.getString(2), x.getString(1))
    }).groupByKey.map(x => {
      //企业ID,保单号不过资料的总共有多少
      (x._1, x._2.size)
    })
    ...
    val end_result: RDD[(String, String, String)] = tep_two.join(tep_three).filter(_._1 != "").map(f = x => {
      //    String ent_material_integrity = "SELECT t1.ent_id,round(1-if(t2.resubmitClaims is NULL,0,t2.resubmitClaims)/t1.claims,4) "
      val ed = x._1 //ent_id
      ...
      (x._1, x._2, "ent_material_integrity")
    })
    //      .take(10).foreach(println(_))
    end_result
  }
  ...
  def avg_aging_cps(employer_liability_claims: DataFrame, ods_policy_detail: DataFrame): RDD[(String, String, String)] = {
    ...
    val ens: RDD[(String, String, String)] = tep_three.map(x => {
      ...
      //      (x._1, numberFormat.format(s), "avg_aging_cps")
  ...
  def disability_num(employer_liability_claims: DataFrame, ods_policy_detail: DataFrame): RDD[(String, String, String)] = {
    ...
    val tepTwo = employer_liability_claims.select("policy_no", "case_type")
    val tepThree = tepOne.join(tepTwo, ods_policy_detail("policy_code") === employer_liability_claims("policy_no"))
      .filter("LENGTH(ent_id)>0").filter("case_type='残疾'")
    //      .show()
    //    |              ent_id|       policy_code|         policy_no|case_type|
  ...
  def worktime_num(employer_liability_claims: DataFrame, ods_policy_detail: DataFrame): RDD[(String, String, String)] = {
    ...
    val tepThree = tepOne.join(tepTwo, ods_policy_detail("policy_code") === employer_liability_claims("policy_no"))
      .filter("LENGTH(ent_id)>0").filter("scene='工作期间'")
    //      .show()
  ...
  def pre_death_compensation(employer_liability_claims: DataFrame, ods_policy_detail: DataFrame): RDD[(String, String, String)] = {
    ...
    val end: RDD[(String, String, String)] = tepThree
      ...
      (x._1, x._2.toString, "pre_death_compensation")
    })
    //      .take(10).foreach(println(_))
    end
  }
  ...
  def pre_dis_compensation(employer_liability_claims: DataFrame, ods_policy_detail: DataFrame): RDD[(String, String, String)] = {
    ...
    val end: RDD[(String, String, String)] = tepThree
      ...
      (x._1, x._2.toString, "pre_dis_compensation")
    })
    //      .take(10).foreach(println(_))
    end
  }
  ...
  def pre_wt_compensation(employer_liability_claims: DataFrame, ods_policy_detail: DataFrame): RDD[(String, String, String)] = {
    ...
    val end = tepThree
      ...
      (x._1, x._2.toString, "pre_wt_compensation")
    })
    //      .take(10).foreach(println(_))
    end
  }
  ...
  def pre_nwt_compensation(employer_liability_claims: DataFrame, ods_policy_detail: DataFrame): RDD[(String, String, String)] = {
    ...
    val end: RDD[(String, String, String)] = tepThree
      ...
      (x._1, x._2.toString, "pre_nwt_compensation")
    })
    end
    //      .take(10).foreach(println(_))
  ...
  def charged_premium(ods_policy_detail: DataFrame, bro_dim: Broadcast[Array[String]], ods_policy_insured_charged: DataFrame): RDD[(String, String, String)] = {
    ...
    val end: RDD[(String, String, String)] = tepThree.select("ent_id", "insure_code", "sku_charge_type", "policy_status", "day_id", "sku_day_price")
      .where("sku_charge_type='2' and policy_status in ('0','1','7','9','10')").filter("LENGTH(ent_id)>0")
      .map(x => {
      ...
      if (bro_dim.value.contains(x._2)) true else false
    }).filter(x => {
      ...
      //ent_id | 没人每天的保费
      (x._1, x._4.toDouble)
    }).union(bf_sum).reduceByKey(_ + _).map(x => {
      //      (x._1, numberFormat.format(x._2), "charged_premium")
      (x._1, x._2.toString, "charged_premium")
    })
    //      .take(10).foreach(println(_))
    end
  }
  ...
  //已赚保费新
  def charged_premium_new(sqlContext: HiveContext, location_mysql_url: String, prop: Properties, ods_policy_detail: DataFrame): RDD[(String, String, String)] = {
    import sqlContext.implicits._
    val ods_policy_charged_month = sqlContext.read.jdbc(location_mysql_url, "ods_policy_charged_month", prop).mapPartitions(par => {
      val now: Date = new Date()
      val dateFormatOne: SimpleDateFormat = new SimpleDateFormat("yyyyMMdd")
      val now_Date = dateFormatOne.format(now)
      par.map(x => {
        (x.getAs[String]("policy_id"),
          x.getAs[java.math.BigDecimal]("charged_premium"),
  ...
  def ent_rejected_count(employer_liability_claims: DataFrame, ods_policy_detail: DataFrame): RDD[(String, String, String)] = {
    ...
    val tepThree = tepOne.join(tepTwo, ods_policy_detail("policy_code") === employer_liability_claims("policy_no"))
      .filter("length(ent_id)>0").filter("status='拒赔'")
    //      .show()
  ...
  def avg_aging_risk(ods_policy_detail: DataFrame, ods_policy_risk_period: DataFrame): RDD[(String, String, String)] = {
    ...
    val end: RDD[(String, String, String)] = tepThree.map(x => {
      (x.getString(0), (x.get(3).toString.toDouble, 1))
    }).reduceByKey((x1, x2) => {
      val sum = x1._1 + x2._1
      val count = x1._2 + x2._2
      (sum, count)
  ...
  def mix_period_rate(ods_policy_detail: DataFrame, ods_policy_risk_period: DataFrame): RDD[(String, String, String)] = {
    ...
    //    |              ent_id|         policy_code|           policy_no|risk_period|
    //    |0a789d56b7444d519...|  900000047702719243|  900000047702719243|         57|
    val total: RDD[(String, Double)] = tepThree.map(x => {
      val risk_period = x.get(3).toString.toDouble
      val ent_id = x.getString(0).toString
      (ent_id, risk_period)
    })
    //求出周期小于3的次数,并求出有多少个
    val threeNumber = total.filter(_._2 < 3.0).map(x => (x._1, 1.0)).reduceByKey(_ + _)
    ...
    //总共有多少个
    val sum_Number = total.map(x => {
      (x._1, 1)
    }).reduceByKey(_ + _).map(x => {
      (x._1, x._2)
    })
    //ent_id | 百分比 |字段
    val end: RDD[(String, String, String)] = threeNumber.join(sum_Number).map(x => {
      //ent_id | 小于3的次数| 总次数
      (x._1, x._2._1, x._2._2)
    }).filter(x => {
      //将次数大于1的过滤出来
      if (x._2 > 1) true else false
    }).map(x => {
      //概率
      val end = x._2 / x._3
  ...
  //工种风险等级入hbase
  def ent_work_risk_r_To_Hbase(ent_work_risk_r: RDD[(String, String, String)], columnFamily1: String, conf_fs: Configuration, tableName: String, conf: Configuration): Unit
  = {
