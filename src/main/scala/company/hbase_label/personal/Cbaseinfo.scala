...
object Cbaseinfo extends until {
  ...
  = {
    val end: RDD[(String, String, String)] = ods_policy_insured_detail.filter("LENGTH(insured_company_name) > 0 and LENGTH(insured_cert_no) > 0")
  ...
  = {
    val rdd = ods_policy_insured_detail.filter("LENGTH(insured_cert_no) > 4 and LENGTH(child_cert_no)>4").select("insured_cert_no", "child_cert_no")
    val end = rdd.map(x => {
      x.getString(0)
    }).distinct.map(x => {
      (x, "是", "user_child")
  ...
  = {
    ...
    val end: RDD[(String, String, String)] = tepThree.map(x => {
      (x.get(2).toString, x.get(1) + "")
      //因为身份证号码是唯一的，因此我去重的话，只取一个即可
    }).reduceByKey((x1, x2) => {
      x1
    }).map(x => {
      (x._1, x._2, "user_craft_level")
    })
    end
  ...
  = {
    //cert_no:被保人身份证号
    val tepOne = employer_liability_claims.filter("length(cert_no)>0").select("cert_no").map(_.getString(0))
    val end: RDD[(String, String, String)] = tepOne.map(x => {
      (x, 1)
    }).reduceByKey(_ + _).map(x => {
      (x._1, x._2 + "", "user_report_num")
    })
    end
  }
  ...
  = {
    val end: RDD[(String, String, String)] = ods_policy_risk_period.filter("LENGTH(cert_no)>0 and risk_period <3").select("cert_no", "risk_period")
      //      .show()
      .map(x => {
      (x.getString(0), 1)
    }).reduceByKey(_ + _).map(x => {
      (x._1, x._2 + "", "user_aging_risk")
    })
    end
  }
  ...
  = {
    ...
    val end = j_after.map(x => {
      val bz = if (x._2.length != null ) x._2 else "null"
