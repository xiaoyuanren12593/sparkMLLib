...
/**
  ...
  */
object Person_Essential_Information extends until {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val user_person = total.mapPartitions(rdd => {
      ...
      val res = rdd.map(x => {
        ...
        //用户类型(user_type) | 骑行天数(ofo_days)  | 保障额度(ofo_guarantee) | 骑行品牌(ofo_brand)
        val product_s = x._2._1.split("\t").map(x => JSON.parseObject(x).getString("code"))
        val version = if (product_s.contains("OFO00001") && !product_s.contains("OFO00002")) {
          val number = count_num(product_s, "OFO00001")
          ("校园版", number + "", "30", "ofo")
        }
        else if (product_s.contains("OFO00002") && !product_s.contains("OFO00001")) {
          val number = count_num(product_s, "OFO00002")
          ("城市版", number + "", "50", "ofo")
        }
        else if (product_s.contains("OFO00001") && product_s.contains("OFO00002")) {
          val number = count_num(product_s, "OFO00002") + count_num(product_s, "OFO00001");
          ("该用户有同时使用过城市版本和校园版本", number + "", "城市版50校园版30", "ofo")
        } else ("null", "null", "null", "非ofo用户")
        ...
        jsonObject_value_before_total.put("user_type", version._1)
        jsonObject_value_before_total.put("ofo_days", version._2)
        jsonObject_value_before_total.put("ofo_guarantee", version._3)
        jsonObject_value_before_total.put("ofo_brand", version._4)
        ...
        //      //Long时间戳和total数据
        //      val date_long = x._2._1.split("\t").map(x => {
        //        val date_long = x.split("~")(0)
        //        (date_long.toLong, x)
        //      })
        //      //求得最近的值
        //      val nearestValue = date_long.reduce((x1, x2) => if (x1._1 >= x2._1) x1 else x2)._2
        //      val personal_Information = s"个人信息:${nearestValue.split("~")(4)},最后一次投保日期:${nearestValue.split("~")(5)},保险止期:${nearestValue.split("~")(6)},社会身份:${nearestValue.split("~")(3)}"
        //
        //      //求得最早的值
        //      val earliest = date_long.reduce((x1, x2) => if (x1._1 <= x2._1) x1 else x2)._2
        //      val fist_now_month_and_product = s"首次投保至今月数:${earliest.split("~")(2)},首次投保产品:${earliest.split("~")(1)},客户来源:${earliest.split("~")(8)}"
        //
        //      //user_insure_code:用户产品号
        //      val user_insure_code = x._2._1.split("\t").map(x => x.split("~")(7)).distinct.mkString("|")
        //      //user_insure_product_num:投保产品数
        //      val num = x._2._2 + ""
        //      //用户类型 | 骑行天数 |骑行保障 | 骑行品牌(ok)
        //      val product_s = x._2._1.split("\t").map(x => x.split("~")(7))
        ...
        //
        //      val user_insure_code_num = s"用户产品号:$user_insure_code,投保产品数:$num,$version"
        //      (x._1, s"$personal_Information,$fist_now_month_and_product,$user_insure_code_num", "user_insure_code_num")

        (x._1, jSONObject.toJSONString, "user_person")
        //        (x._1, jsonObject_value, jsonObject_value_before, jsonObject_value_before_total)
      })
      res
    })

    toHbase(user_person, columnFamily1, "user_person", conf_fs, tableName, conf)

    //    val personal_Information = user_person.map(x => (x._1, x._2.toJSONString, "personal_Information"))
    //    val fist_now_month_and_product = user_person.map(x => (x._1, x._3.toJSONString, "fist_now_month_and_product"))
    //    val user_insure_code_num = user_person.map(x => (x._1, x._4.toJSONString, "user_insure_code_num"))
    //
