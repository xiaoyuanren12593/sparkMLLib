...
object tep_one extends piwik_until {
  System.setProperty("HADOOP_USER_NAME", "hdfs")

  //字段与字段值合并
  def merge(df: DataFrame): (RDD[Row], StructType)
  = {
    val columns: Seq[String] = df.schema.map(x => x.name).filter(!_.contains("custom_var")) //过滤完后剩余19个字段
  ...
  = {
    ...
    //    //订单ID将其广播
    //    val order_bro = sc.broadcast(
    //      ods_policy_detail.where("policy_status in ('0','1','7','9','10')")
