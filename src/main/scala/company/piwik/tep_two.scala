...
/**
  ...
  */
object tep_two extends piwik_until {
  System.setProperty("HADOOP_USER_NAME", "hdfs")

  def loadToHive(sqlContext: HiveContext): DataFrame
  = {
    //得到piwik_log_action可找到type
    val piwik_log_action = sqlContext.sql("select * from piwik_vt.piwik_log_action").map(x => {
    ...
    //大表生成DF
    val vt = sqlContext.createDataFrame(value_tepTwo, schema_tepOne).withColumnRenamed("idsite", "idsite_vt").withColumnRenamed("idvisitor", "idvisitor_vt").withColumnRenamed("idvisit", "idvisit_vt").persist(StorageLevel.MEMORY_AND_DISK)
    ...
    val fields_name_value: DataFrame = end.map(x => {
      ...
      (idvisit, idvisitor, visit_entry_idaction_url, visit_entry_idaction_url_name,
        ...
        idaction_event_action_name, referer_keyword, referer_name, referer_type, visit_last_action_time, visit_first_action_time, visitor_localtime, name)
    }).toDF(end_fields: _*)
