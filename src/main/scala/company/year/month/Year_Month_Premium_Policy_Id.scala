...
/**
  ...
  */
object Year_Month_Premium_Policy_Id {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val bzn_year: RDD[(String, Double)] = ods_policy_insured_charged_vt.map(x => {
    ...
    }).filter(_._3.toInt <= get_now_time).map(x => (x._1, x._2.toDouble)).reduceByKey(_ + _)
