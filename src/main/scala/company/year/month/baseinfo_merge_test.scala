...
object baseinfo_merge_test extends year_until {
  ...
  = {
    ...
    val b_policy_preservation_subject_person_master_version_one: DataFrame = sqlContext.sql("select " +
      ...
      "work_type as c_work_type,case when insured_cert_type ='1' and insured_start_date is not null then getAge(a_insured_cert_no,insured_start_date) else null end as age from mk_ce").drop("work_type") //.persist(StorageLevel.MEMORY_ONLY_SER)
